...
import { bindSnapshotSelector, RemoteError, stubSettingsScope } from '@deepseek-ai/dsh-client-test-runtime'
import type {
...
function routingProps(): Pick<ModelsSectionProps, 'useRouting' | 'selectRouting'> {
  const routing = stubSettingsScope<ModelRoutingSettings>()
  routing.publish({ status: 'ready', value: { main: 'deepseek-v4-flash' }, writable: true })
  return {
    useRouting: bindSnapshotSelector(routing.scope),
    selectRouting: vi.fn(),
  }
}
...
function scriptedFace(overrides: {
  ...
  const face = {
    ...
    session: {
      modelCatalog: vi.fn(() => Promise.resolve(remoteOk({ groups: [], failures: [] }))),
    },
    settings: {
