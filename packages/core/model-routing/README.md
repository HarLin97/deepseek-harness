...
Two-tier model routing (main Agent / sub-agent) as a `model-routing` Settings namespace. The plugin registers the namespace when a settings provider is composed; the browser settings scope reads and writes the same section, and the Host consumer (sub-agent launch) reads it through `ctx.settings`.

The section carries `{ main, sub? }`:
...
- `ctx.settings.get(MODEL_ROUTING_NAMESPACE)` returns the resolved `{ main, sub? }` section.
...
Indirectly, through the model ids it resolves: the namespace only stores and resolves both tiers, and the routing consumer (sub-agent launch) owns every model-visible request it builds from a resolved id.
...
- The namespace is composed into the base profile layer, so its section ships in the harness bundle.
- Image-bearing requests are out of scope: upstream owns multimodal admission, so there is no vision tier to route them through.
