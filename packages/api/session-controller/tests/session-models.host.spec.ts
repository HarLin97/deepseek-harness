...
import { MODEL_ROUTING_NAMESPACE } from '@deepseek-ai/dsh-model-routing'
...
describe('Web session model selection', () => {
  ...
  it('admits an image through the configured vision model while the current model is text-only', async () => {
    const { ctx, agent, sessionId } = await harness()
    ctx.llm.registerAdapter(['routed-vision'], new class extends CatalogAdapter {
      override resolveModel(provider: string, model: string): Promise<LlmResolvedModelInfo> {
        return Promise.resolve({
          provider,
          id: model,
    ...
    ctx.provide('settings', {
      get: (namespace: unknown) => namespace === MODEL_ROUTING_NAMESPACE
        ? { main: 'plain', vision: 'vision' }
        : undefined,
    } as never)
    const savedRef = {
      attachmentId: 'routed-image',
    ...
    ctx.provide('attachments', {
      saveImages: vi.fn(() => Promise.resolve([savedRef])),
    } as never)
    const followup = vi.fn()
    Object.assign(agent, { followup })
    const remote = createSessionTestRemote(ctx, {
      defaultModelSelection: () => ({ provider: 'routed-vision', model: 'plain' }),
      cwd: '/tmp',
    })

    expectValue(await remote.selectModel(request({
      sessionId,
      provider: 'routed-vision',
      model: 'plain',
    })))
