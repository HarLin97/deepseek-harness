...
/**
 * Whether an image prompt can use the selected route or its configured vision tier.
 * @param ctx - Host context carrying LLM and optional settings services.
 * @param selection - Session-local route selected before admission.
 * @returns true when either exact model advertises image input.
 */
async function imageRouteAvailable(
  ctx: Context,
  selection: AgentModelSelection,
): Promise<boolean> {
  const selected = await ctx.llm.resolveModelInfo(selection.provider, selection.model)
  if (selected.inputModalities?.includes('image') === true) return true
  const routing = ctx.get('settings')?.get(MODEL_ROUTING_NAMESPACE) as ModelRoutingSettings | undefined
  const vision = routing === undefined ? undefined : resolveVision(routing.main, routing.vision)
  if (vision === undefined) return false
  try {
    const target = await ctx.llm.resolveModelInfo(selection.provider, vision)
    return target.inputModalities?.includes('image') === true
  } catch {
    return false
  }
}
    ...
    const admit = async (): Promise<SessionPromptValue> => {
      try {
        if (hasImage) {
          ...
          if (!await imageRouteAvailable(this.ctx, current)) {
            throw new RemoteError(
              'session/attachment-invalid',
              `Model "${current.model}" does not support image input, and no configured vision model can serve it.`,
