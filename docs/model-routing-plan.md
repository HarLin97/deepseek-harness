...
- **后续修正（全局视觉路由）**：上述 per-agent 视觉 listener 只对 api-proxy 组装的 agent 注册——web 聊天等由其他入口直接 `agents.create` 的 agent 没有该 listener，含图请求直达 adapter 报 `UNSUPPORTED_CONTENT`。修正：视觉路由改为 GLOBAL——`@deepseek-ai/dsh-model-routing` 的 `apply` 在根上下文注册 `agent/request` listener（根上下文无 scope 标签，按 harness 事件作用域规则对每个 agent 的水瀑都放行，且启动期注册使其恒为最外层，改写定稿在 selection 盖章之上）；apiproxy 里 `selectionFor` 的 per-agent 视觉 listener 移除，准入拒绝（RPC 路径无可用 vision tier 时报 vision-not-supported）保留。含图 + 无视觉能力 + vision 已配 → 该次请求用 vision；含图 + 模型有视觉能力 → 不变；无图 → 不变；含图 + vision 未配 → 抛 `LlmError`（`MODEL_DOES_NOT_SUPPORT_IMAGES`，文案含 "vision is not supported"），web 聊天含图请求明确拒绝而不是放给 adapter。单测：model-routing 8 个新用例直接驱动 `agent/request` 水瀑覆盖四类路径与边界（无 settings、无 llm、路由不可解析、inputModalities 缺失），apiproxy 6 个既有用例保持 RPC 准入与最外层改写回归。
